...
用法：
  python test_data_multifolder_jsonl_flow.py --jsonl /path/to/reads.jsonl.gz
  python test_data_multifolder_jsonl_flow.py --jsonl /path/to/reads.jsonl
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=64,
        help="How many tokens to preview for input_ids/attention_mask per sample when not showing full tensors.",
    )
    ap.add_argument(
        "--show_full_tensors",
        action="store_true",
        help="If set, print full input_ids and attention_mask tensors (can be very long).",
    )
    args = ap.parse_args()
    ...
    if args.show_full_tensors:
        print("  input_ids(full):", batch["input_ids"].tolist())
        print("  attention_mask(full):", batch["attention_mask"].tolist())
    else:
        preview_n = max(1, int(args.preview_tokens))
        input_ids_preview = [row[:preview_n] for row in batch["input_ids"].tolist()]
        attention_mask_preview = [row[:preview_n] for row in batch["attention_mask"].tolist()]
        print(f"  input_ids(first {preview_n} tokens per sample):", input_ids_preview)
        print(f"  attention_mask(first {preview_n} tokens per sample):", attention_mask_preview)
