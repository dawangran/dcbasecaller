...
def nccl_socket_preflight() -> Tuple[bool, Optional[str]]:
    ...
    def _non_loopback(names: List[str]) -> List[str]:
        return [name for name in names if name != "lo" and not name.startswith("lo:")]

    if os.environ.get("NCCL_SOCKET_IFNAME"):
        raw_spec = os.environ["NCCL_SOCKET_IFNAME"]
        include, exclude = [], []
        ...
        if include:
            matched = [name for name in include if name in iface_set]
            if matched:
                return True, None
            return False, f"NCCL_SOCKET_IFNAME={raw_spec!r} does not match any visible network interface"

        allowed = [name for name in iface_names if name not in set(exclude)]
        if _non_loopback(allowed):
            return True, None
        return False, f"NCCL_SOCKET_IFNAME={raw_spec!r} excludes all visible non-loopback interfaces"

    if _non_loopback(iface_names):
        return True, None
