...
- `--decoder`: choose `ctc_greedy`, `ctc_viterbi`, `koi`, or `ctc_crf` for prediction/metrics.
...
- `--decoder`: choose `ctc_greedy`, `ctc_viterbi`, `koi`, or `ctc_crf` for prediction; CTC-CRF forces fp32 decoding.
- `--head_type`: optional override for checkpoint head type (`ctc` or `ctc_crf`, default auto-infer).
